...
let IsNotExistTag=(IsExist,tagname) => {
    return IsExist ?  `This HTML have <${tagname}> tag.` :`This HTML not has <${tagname}> tag.`;
}
...
let IsheadContentCheck= (checkoobj={title: false,meta_keyword: false,meta_descriptions:false}) =>{

    let titlelog="", descriptionslog="", keywordslog="";
    titlelog = checkoobj.title ?  `This head have <title> tag.` :`This head haven't <title> tag.`;
    descriptionslog=checkoobj.meta_descriptions ?  `This head have <meta name='descriptions'> tag.` :`This head haven't <meta name='descriptions'> tag.`;
...
let savelog = (filepath,filename,filecontnt,callback=undefined)=>{
